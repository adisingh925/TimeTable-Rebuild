...
import com.app.adreal.timetable.databinding.FragmentFridayBinding
import com.app.adreal.timetable.daysadapter.fridayadapter
import com.app.adreal.timetable.daysdatabase.daysviewmodel.fridayViewModel
...
class friday : Fragment(), fridayadapter.OnItemClickListener {

    lateinit var binding: FragmentFridayBinding

    lateinit var fridayViewModel: fridayViewModel
    ...
    override fun onCreateView(
        ...
        fridayViewModel = ViewModelProvider(this).get(com.app.adreal.timetable.daysdatabase.daysviewmodel.fridayViewModel::class.java)

        val adapter = fridayadapter(this)
        ...
        fridayViewModel.readalldata.observe(viewLifecycleOwner, androidx.lifecycle.Observer { data ->
    ...
    private fun showcustomdialog() {
        ...
        dialog.findViewById<Button>(R.id.done).setOnClickListener()
        {
            ...
            {
                val data = friday_model(0,auth.uid.toString(),dialog.findViewById<EditText>(R.id.starttime).text.toString(),dialog.findViewById<EditText>(R.id.endtime).text.toString(),dialog.findViewById<EditText>(R.id.subject).text.toString())
                fridayViewModel.insert(data)
                dialog.dismiss()
            }
            else
            {
    ...
    override fun onItemClick(data: friday_model) {
        fridayViewModel.delete(data)
    }
}